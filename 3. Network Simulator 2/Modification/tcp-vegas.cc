...
int VegasTcpAgent::command(int argc, const char*const* argv)
{
	if (strcmp(argv[1], "call_showVar") == 0)
    {
       showVar();	// invoke the corresponding implementation
       return(TCL_OK);
    }
    return(TcpAgent::command(argc, argv));
}

void VegasTcpAgent::showVar(void)
{
	printf("*** %d *****\n",newVegas);
}
...
VegasTcpAgent::output(int seqno, int reason)
{
	...
	if(newVegas) //samu
	{
		if(cwnd_ < ssthresh_ && v_cntRTT_>0)
		{
			if(seqno>0 && seqno%2)
			{
				double delay= v_baseRTT_/cwnd_;
