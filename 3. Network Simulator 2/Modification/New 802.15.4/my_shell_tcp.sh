...
avg="avg"
tracef="trace_tcp_wireless_static.tr"
tclf="tcp_802_15_4_wireless_static.tcl"
awkf="my_awk_tcp.awk"
...
do
	...
	do
		...
		gnuplot -persist -e "set title '802.15.4 mod : ${metric[j]} vs ${var[i]}'; set xlabel '${var[i]}'; set ylabel '${metric[j]}';  set terminal png size 800,600; set output '$pic_file'; plot '$graph' using 1:$temp with lines;"
		j=$(($j+1))
		t=$(($t+1))
	done
	i=$(($i+1))
